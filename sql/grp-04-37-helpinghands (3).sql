...
-- Procedures
--
DROP PROCEDURE IF EXISTS `add_into_city`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_into_city` (IN `city_name` VARCHAR(255), IN `city_abr` VARCHAR(5))   BEGIN
...
DROP PROCEDURE IF EXISTS `assigned_patients_with_chronic_conditions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `assigned_patients_with_chronic_conditions` (IN `nurse_ID` INT(10))   BEGIN
...
DROP PROCEDURE IF EXISTS `care_visits_for_patient`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `care_visits_for_patient` (IN `nurse_ID` INT, IN `care_contract_type` VARCHAR(10), IN `patient_ID` INT)   BEGIN
...
DROP PROCEDURE IF EXISTS `delete_into_city`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_city` (IN `city_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_condition`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_condition` (IN `condition_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_nurse_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_nurse_profile` (IN `nurse_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_patient_chronic_conditions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_patient_chronic_conditions` (IN `patient_chronic_conditions_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_patient_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_patient_profile` (IN `patient_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_prefered_nurse_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_prefered_nurse_suburb` (IN `prefered_nurse_suburb_ID_new` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `delete_into_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_into_suburb` (IN `suburb_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `GetPatientProfileInfo`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetPatientProfileInfo` (IN `patientID` INT)   BEGIN
    DECLARE userEmail VARCHAR(255);
    DECLARE userContactNo INT;
    DECLARE suburbName VARCHAR(255);
    ...
    JOIN
        suburb s ON pp.suburb_ID = s.suburb_ID
    WHERE
        pp.patient_ID = patientID;

    SELECT userEmail AS 'Email Address', userContactNo AS 'Contact Number', suburbName AS 'Suburb Name';
END$$

DROP PROCEDURE IF EXISTS `get_care_contract_by_contract_status`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_care_contract_by_contract_status` (IN `contract_status` VARCHAR(1), IN `care_contract_start_date` DATE, IN `care_contract_end_date` DATE)   BEGIN
...
DROP PROCEDURE IF EXISTS `get_list_of_nurses_per_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_list_of_nurses_per_suburb` (IN `suburb_ID` INT(10))   SELECT
...
DROP PROCEDURE IF EXISTS `get_patient_care_contracts_and_visits`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_patient_care_contracts_and_visits` (IN `patient_ID` INT(10), IN `contract_status` VARCHAR(10))   BEGIN
...
DROP PROCEDURE IF EXISTS `insert_condition`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_condition` (IN `condition_name_NEW` VARCHAR(255), IN `condition_description_NEW` VARCHAR(550))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `insert_into_nurse_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_into_nurse_profile` (IN `user_ID_NEW` INT, IN `suburb_ID_NEW` INT, IN `first_name_NEW` VARCHAR(255), IN `last_name_NEW` VARCHAR(255), IN `nurse_code_NEW` INT, IN `gender_NEW` VARCHAR(2), IN `id_number_NEW` INT, IN `dob_NEW` DATE)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `insert_into_patient_chronic_conditions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_into_patient_chronic_conditions` (IN `patient_ID_NEW` INT, IN `condition_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `insert_into_patient_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_into_patient_profile` (IN `user_ID_NEW` INT, IN `suburb_ID_NEW` INT, IN `first_name_NEW` VARCHAR(255), IN `last_name_NEW` VARCHAR(255), IN `gender_NEW` VARCHAR(2), IN `id_number_NEW` INT, IN `dob_NEW` DATE, IN `ec_person_NEW` VARCHAR(255), IN `ecp_number_NEW` INT, IN `profile_picture_NEW` VARCHAR(255), IN `about_patient_NEW` VARCHAR(255), IN `street_name_add_NEW` VARCHAR(255), IN `fb_link_NEW` VARCHAR(255), IN `insta_link_NEW` VARCHAR(255))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
  ...
  gender, id_number, dob, ec_person, ecp_number, profile_picture, about_patient, street_name_add, fb_link, insta_link)
  VALUES (user_ID_NEW, suburb_ID_NEW, first_name_NEW, last_name_NEW, 
  gender_NEW, id_number_NEW, dob_NEW, ec_person_NEW, ecp_number_NEW, profile_picture_NEW, about_patient_NEW, street_name_add_NEW, fb_link_NEW, insta_link_NEW);

  COMMIT;
END$$

DROP PROCEDURE IF EXISTS `insert_into_prefered_nurse_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_into_prefered_nurse_suburb` (IN `user_ID_NEW` INT, IN `suburb_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `insert_into_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_into_suburb` (IN `suburb_name` VARCHAR(255), IN `postal_code` VARCHAR(10), IN `city_ID` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `list_care_visits_per_nurse`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `list_care_visits_per_nurse` (IN `vsit_date_from` DATE, IN `vsit_date_to` DATE, IN `nurse_ID` INT)   BEGIN
    -- Validate START and END DATES
...
DROP PROCEDURE IF EXISTS `patient_care_contracts_assigned`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `patient_care_contracts_assigned` (IN `nurse_ID` INT)   BEGIN
    SELECT
        cc.contract_number,
        cc.nurse_ID,
        cc.patient_ID,
...
DROP PROCEDURE IF EXISTS `read_into_city`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_city` (IN `city_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `read_into_condition`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_condition` (IN `condition_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `read_into_nurse_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_nurse_profile` (IN `nurse_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `read_into_patient_chronic_conditions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_patient_chronic_conditions` (IN `patient_chronic_conditions_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `read_into_patient_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_patient_profile` (IN `user_ID_NEW` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
   ...
   SELECT patient_profile.*, suburb.suburb_ID, suburb.name
    FROM patient_profile patient_profile
    JOIN suburb suburb ON patient_profile.suburb_ID = suburb.suburb_ID
    WHERE patient_profile.user_ID = user_ID_NEW;

END$$

DROP PROCEDURE IF EXISTS `read_into_prefered_nurse_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_prefered_nurse_suburb` (IN `prefered_nurse_suburb_ID_new` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `read_into_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `read_into_suburb` (IN `suburb_ID_new` INT)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `test_procedure` (IN `test_ID_NEW` INT(11), IN `test_name_NEW` VARCHAR(255))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
    ...
    VALUES (test_ID_NEW, test_name_NEW);


    COMMIT;
END$$

DROP PROCEDURE IF EXISTS `update_into_city`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_city` (IN `city_ID_NEW` INT, IN `new_city_name` VARCHAR(255), IN `new_city_abr` VARCHAR(5))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
  BEGIN
    ROLLBACK;
    SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `update_into_condition`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_condition` (IN `condition_name_NEW` VARCHAR(255), IN `condition_description_NEW` VARCHAR(550))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `update_into_nurse_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_nurse_profile` (IN `nurse_ID_NEW` INT, IN `user_ID_NEW` INT, IN `suburb_ID_NEW` INT, IN `first_name_NEW` VARCHAR(255), IN `last_name_NEW` VARCHAR(255), IN `nurse_code_NEW` INT, IN `gender_NEW` VARCHAR(2), IN `id_number_NEW` INT, IN `dob_NEW` DATE)   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `update_into_patient_chronic_conditions`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_patient_chronic_conditions` (IN `patient_chronic_conditions_ID_NEW` INT, IN `patient_ID_NEW` INT, IN `condition_ID_NEW` INT, IN `status_new` VARCHAR(1))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `update_into_patient_profile`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_patient_profile` (IN `patient_ID_NEW` INT, IN `user_ID_NEW` INT, IN `suburb_ID_NEW` INT, IN `first_name_NEW` VARCHAR(255), IN `last_name_NEW` VARCHAR(255), IN `gender_NEW` VARCHAR(1), IN `id_number_NEW` INT, IN `dob_NEW` DATE, IN `ec_person_NEW` VARCHAR(255), IN `ecp_number_NEW` INT, IN `profile_picture_NEW` VARCHAR(255), IN `street_name_add_NEW` VARCHAR(255), IN `fb_link` VARCHAR(255), IN `insta_link` VARCHAR(255), IN `about_patient_NEW` VARCHAR(255))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
    ...
    UPDATE patient_profile
    SET user_ID = user_ID_new, suburb_ID = suburb_ID_new, first_name = first_name_NEW, last_name = last_name_NEW, gender = gender_NEW, id_number = id_number_NEW, dob = dob_NEW, ec_person = ec_person_NEW, ecp_number = ecp_number_NEW, profile_picture = profile_picture_NEW, about_patient = about_patient_NEW, street_name_add = street_name_add_NEW, fb_link = fb_link_NEW, insta_link = insta_link_NEW
    WHERE patient_ID = patient_ID_NEW;

    COMMIT;
END$$

DROP PROCEDURE IF EXISTS `update_into_prefered_nurse_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_prefered_nurse_suburb` (IN `prefered_nurse_suburb_ID_new` INT, IN `user_ID_new` INT, IN `suburb_ID_new` INT, IN `status_new` VARCHAR(1))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP PROCEDURE IF EXISTS `update_into_suburb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_into_suburb` (IN `suburb_ID_NEW` INT, IN `city_ID_new` INT, IN `new_suburb_name` VARCHAR(255), IN `new_postal_code` INT(10), IN `new_status` VARCHAR(1))   BEGIN
  DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000'
...
DROP TABLE IF EXISTS `care_contract`;
CREATE TABLE IF NOT EXISTS `care_contract` (
  ...
  KEY `fk_patient_contract` (`patient_ID`),
  KEY `fk_nurse_contract` (`nurse_ID`),
  KEY `fk_suburb_contract` (`suburb_ID`)
...
(8, 6, 2, 14, 'test', '2023-08-01', '2023-08-02', '2023-09-01', 'A');
...
CREATE TABLE IF NOT EXISTS `care_visit` (
  `care_visit_ID` int UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`care_visit_ID`),
  KEY `fk_patient_care_visit` (`patient_ID`),
  KEY `fk_nurse_care_visit` (`nurse_ID`),
  KEY `fk_contract_number_care_visit` (`contract_number`)
...
CREATE TABLE IF NOT EXISTS `nurse_profile` (
  ...
  KEY `fk_nurse_profile_user` (`user_ID`),
  KEY `fk_nurse_profile_suburb` (`suburb_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `password_reset_tokens` (
  `password_reset_tokens_ID` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_ID` int UNSIGNED NOT NULL,
  `email_address` varchar(255) NOT NULL,
  `token` varchar(255) NOT NULL,
  `created_at` datetime NOT NULL,
  PRIMARY KEY (`password_reset_tokens_ID`),
  KEY `fk_user_ID_pass_reset` (`user_ID`)
...
CREATE TABLE IF NOT EXISTS `patient_chronic_conditions` (
  `patient_chronic_conditions_ID` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `patient_ID` int UNSIGNED NOT NULL,
  `condition_ID` int UNSIGNED NOT NULL,
  `status` char(1) NOT NULL DEFAULT 'A',
  PRIMARY KEY (`patient_chronic_conditions_ID`),
  KEY `fk_user_chronic_id` (`patient_ID`),
  KEY `fk_user_chronic_condition` (`condition_ID`)
...
CREATE TABLE IF NOT EXISTS `patient_profile` (
  ...
  `about_patient` varchar(255) NOT NULL,
  `street_name_add` varchar(255) NOT NULL,
  `fb_link` varchar(255) NOT NULL,
  `insta_link` varchar(255) NOT NULL,
  PRIMARY KEY (`patient_ID`),
  KEY `fk_patient_profile_user` (`user_ID`),
  KEY `fk_patient_profile_suburb` (`suburb_ID`)
...
INSERT INTO `patient_profile` (`patient_ID`, `user_ID`, `suburb_ID`, `first_name`, `last_name`, `gender`, `id_number`, `dob`, `ec_person`, `ecp_number`, `status`, `profile_picture`, `about_patient`, `street_name_add`, `fb_link`, `insta_link`) VALUES
...
(6, 19, 6, 'Pitso', 'Mana', 'M', 2147483647, '1996-10-30', 'Pamela Kilane', 671023499, 'A', '#', 'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry\'s standard dummy', '20 mzwandile street', 'www.facebook.com', 'www.instagram.com');
...
DROP TABLE IF EXISTS `prefered_nurse_suburb`;
CREATE TABLE IF NOT EXISTS `prefered_nurse_suburb` (
  `prefered_nurse_suburb_ID` int UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_ID` int UNSIGNED NOT NULL,
  `suburb_ID` int UNSIGNED NOT NULL,
  `status` char(1) NOT NULL DEFAULT 'A',
  PRIMARY KEY (`prefered_nurse_suburb_ID`),
  KEY `fk_pre_nurse` (`user_ID`),
  KEY `fk_pre_nurse_suburb` (`suburb_ID`)
...
(19, 'PatientTest', 'patient.test@gmail.com', '$2y$10$U9M3qyKFXUzfmR9ahICxI.YVn6JjKvbFPvxKsxqW1xOzUKFKaoEeq', 2147483647, 'P', 'A', '2023-09-26 12:51:54', 0);
